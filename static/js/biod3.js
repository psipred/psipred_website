...
var data = null;
//takes a sequence string and returns an array of, annotation maps
function process_sequence_string(string)
{
  var seq_array = string.split("");
  var results = [];
  for(var i=0;i<seq_array.length;i++)
  {
    var data = {name: seq_array[i]};
    results[i] = data;
  }
  return(results);
}

//takes amn array of annotation maps and adds a set of annotation
function add_annotation(data, annotation, label)
{
  for(var i=0; i<data.length; i++)
  {
    data[i][label] = annotation[i];
  }
  return(data);
}

function add_X_scale(panel, options, grid_wrap, em_size, width, data_length, number_of_lines, labelled_axes, x_labels, location)
{
  var lower_offset = Math.floor(data_length/grid_wrap)*grid_wrap;
  var dom_start = grid_wrap/10;
  var dom_stop = grid_wrap;
  var initial_y = 0;
  var x = null;
  var xAxis = null;
  var rotation_direction = 65;

  if(location === "bottom")
  {
    dom_start = lower_offset+(grid_wrap/10);
    dom_stop = lower_offset+grid_wrap;
    initial_y = em_size*number_of_lines;
    rotation_direction = -65;
  }
  if(labelled_axes === false){
    x = d3.scale.linear()
      .domain([dom_start, dom_stop])
      .range([em_size*5-(em_size/2), width-(em_size*5)+(em_size/2)]);
    xAxis = d3.svg.axis()
        .scale(x)
        .orient(location);
    panel.append("g")
  ...
  else
  {
    x = d3.scale.ordinal()
      .domain(x_labels)
      .rangeBands([0, (em_size*data_length)]);
    xAxis = d3.svg.axis()
          .scale(x)
          .tickSize(0)
          .orient(location);
    panel.append("g")
        .attr("class", "x axis "+location)
        .attr("transform", "translate(-"+em_size/2+" ,"+initial_y+")")
        .attr("font-size", (em_size/2))
        .attr("font-family", "sans-serif")
        .call(xAxis)
          .selectAll("text")
          .style("text-anchor", "end")
            .attr("transform", "rotate("+rotation_direction+")");
  }
}

function add_Y_scale(panel, grid_wrap, number_of_lines, em_size, labelled_axes, y_labels, location)
{
  var x_offset = 0;
  var y_offset = em_size*0.5;
  var domain = new Array(number_of_lines);
  var y = null;
  var yAxis = null;

  for(var i=0;i<domain.length;i++){
    domain[i] = (i*grid_wrap) + 1;
  }
  domain.reverse();

  if(location === "right")
  {
    var right_domain = domain;
    for(i=0;i<right_domain.length;i++){
      right_domain[i] = right_domain[i]+(grid_wrap-1);
    }
    domain=right_domain;
    x_offset = em_size*grid_wrap;
    y_offset = em_size*0.4;
  }
  if(labelled_axes === false){
    y = d3.scale.ordinal()
      .domain(domain)
      .rangePoints([em_size*(number_of_lines-1), 1]);
    yAxis = d3.svg.axis()
    .scale(y)
...
function add_annotation_key(panel, data, cell_class, width, margin, number_of_lines, em_size, bottom_x_pad,heatmapColour, colour_scale, user_colours)
{
  var classes = null;
  if(cell_class === "ss")
  {
    classes = [{label:"Helix", ss: "H"},{label:"Strand", ss: "E"},{label:"Turn", ss: "T"},{label:"Coil", ss: "C"}];
  }
  if(cell_class === "clustal")
  {
    classes = [{name:"A", label: "Small"},{name:"C", label: "Hyrdophobic"},
               {name:"D", label: "Charged"},{name:"F", label: "Aromatic"},{name:"B", label:"Other"}];
  }
  if(cell_class === "lesk")
  {
    classes = [{name:"A", label: "Small non-polar"},{name:"N", label: "Acidic"},
               {name:"C", label: "Hydrophobic"},{name:"D", label: "Negative"},{name:"B", label:"Other"}];
  }
  if(cell_class === "heatmap")
  {
    data_extent = d3.extent(data, valueFn);
    classes = [{label:"Lowest", value: data_extent[0]},{label:"Highest", value: data_extent[1]}];
  }
  if(user_colours !== null && cell_class in user_colours)
  {
    user_classes = user_colours[cell_class];
    classes = [];
    for(var k in user_classes)
    {
      classes.push({label: k, value: user_classes[k]});
    }
  }

  annotation_panel_segment_size = (width-margin.right-margin.left)/5;
  //panel_left_offset=
  panel_bottom_offset=(number_of_lines+2)*em_size;
  var annotation_cells = panel.selectAll(".item")
      .data(classes)
      .enter().append("g")
        .attr("class", "annotation_cell")
        .attr("transform", function(d, i) {
          var row_idx = Math.floor(i/5);
          x_offset = (i * em_size) + (i*annotation_panel_segment_size);
          y_offset = panel_bottom_offset;
          return("translate("+x_offset+","+y_offset+")");
        });
  text_x_offset = 1.5*em_size;
  text_y_offset = (em_size / 2)-(em_size/100);
  place_cells(annotation_cells, em_size, cell_class, user_colours, heatmapColour, colour_scale);

  annotation_cells.append("text")
...
function place_cells(cells, em_size, cell_class, user_colours, heatmapColour, colour_scale, text_x_offset, text_y_offset)
{
      if(user_colours !== null && cell_class in user_colours)
      {
        console.log("hello");
        cells.append("rect")
            ...
            .attr("fill", function(d) {return customValue(d, user_colours, cell_class);})
            .attr("id", function(d, i){return("pos_"+i);});
      }
      else if(cell_class === "heatmap")
      {
        cells.append("rect")
            .attr("width", em_size)
            .attr("height", em_size)
            ...
            .attr("fill", function(d) {
              if(valueFn(d) !== undefined){
                return heatmapColour(colour_scale(valueFn(d)));
              }
              else {
                return("#ffffff");
              }
        ...
        cells.append("rect")
            ...
            .attr("fill", function(d) {
              if(ssFn(d) !== undefined){
                if(ssFn(d) === "H"){return("#ff99ff");}
                else if(ssFn(d) === "E"){return("#eeee44");}
                else if(ssFn(d) === "T"){return("#6080ff");}
                else {return("#ffffff");}
              }
              else {
                return("#ff0000");
        ...
        var Clustal = schemeMgr.getScheme(cell_class);
        cells.append("rect")
            .attr("width", em_size)
            .attr("height", em_size)
            .attr("class", cell_class)
            .attr("fill", function(d) {
              if(nameFn(d) !== undefined){
                return Clustal.getColor(nameFn(d));
              }
              else {
                return("#ffffff");
              }
...
function add_annotation_selector(data, svg_div_class, cell_class, panel_name, data_name)
{
  ...
  annotation_keys = Object.keys(entry);

  var local_panel = d3.select("."+svg_div_class);
  local_panel.selectAll(".annotation_selector").remove();

    local_panel.append('div')
          .attr('class', 'annotation_selector')
       .append("form")
       .selectAll(".annotation_select")
        .data(annotation_keys)
        .enter()
        .append("label")
...
function valueFn(d) { return d.value; }
function ssFn(d) { return d.ss; }
function nameFn(d) { return d.name; }
function labelFn(d) { return d.label; }
function customValue(d, user_colours, cell_class){
  if("label" in d){return d.value;}
  if(cell_class in d){
    if(cell_class in user_colours)
    {
      if(d[cell_class].length === 0)
      {
        return user_colours[cell_class].blank;
      }
      else
      {
        return user_colours[cell_class][d[cell_class]];
      }
    }
  }
}
...
//Builds which can be annotated, useful for heat maps and whatnot
function bio_panel(d, gw, svg_element, opt)
{
    //draw the panel
    data = d;
    var that = {},
        svg_div_class = svg_element,
        grid_wrap = gw,
        data_length = data.length,
        number_of_lines = Math.ceil(data_length/grid_wrap),
        em_size = getEmSize("body"),
        top_x_pad = 2, //   these control the padding fraction that we use to
        bottom_x_pad = 2,// calculate the margins
        left_y_pad = 2,
        right_y_pad = 2,
        left_domain = null,
        right_domain = null,
        ...
        user_colours = null
        if(opt !== undefined){
          if(opt.topX !== undefined){options.topX = opt.topX;}
          if(opt.bottomX !== undefined){options.bottomX = opt.bottomX;}
          if(opt.leftY !== undefined){options.leftY = opt.leftY;}
          if(opt.rightY !== undefined){options.rightY = opt.rightY;}
          if(opt.cellClass !== undefined){cell_class = opt.cellClass;}
          if(opt.labelled_axes !== undefined){labelled_axes = opt.labelled_axes;}
          if(opt.x_labels !== undefined){x_labels = opt.x_labels;}
          if(opt.y_labels !== undefined){y_labels = opt.y_labels;}
    ...
    if(options.bottomX === false){bottom_x_pad = 0;}
    if(options.leftY === false){left_y_pad = 0;}
    if(options.rightY === false){right_y_pad = 0;}
    if(annotation_key === true){bottom_x_pad += 2;}
    var height = (number_of_lines*em_size)+(em_size*(top_x_pad+bottom_x_pad)),
        width = grid_wrap*em_size+(em_size*(left_y_pad+right_y_pad)),
        margin = {top: em_size*top_x_pad, right: em_size*right_y_pad, bottom: em_size*bottom_x_pad, left: em_size*left_y_pad};

    var heatmapColour = null;
    var colour_scale = null;
    if(cell_class === "heatmap")
    {  var colours = ["red", "white", "green"];
       heatmapColour = d3.scale.linear()
         .domain(d3.range(-1, 1, 1 / (colours.length - 1)))
         .range(colours);
       colour_scale = d3.scale.linear()
           .domain(d3.extent(data, valueFn))
           .range([-1,0]);
    }

    local_panel = d3.select("."+svg_div_class)
         .append('svg')
          .attr('height', height)
          .attr('width', width)
         .append('g')
    ...
    {
      add_X_scale(local_panel, options, grid_wrap, em_size, width, data_length, number_of_lines, labelled_axes, x_labels, "top");
    }
    if(options.bottomX !== false)
    {
      add_X_scale(local_panel, options, grid_wrap, em_size, width, data_length, number_of_lines, labelled_axes, x_labels, "bottom");
    }
    if(options.leftY !== false)
    {
      add_Y_scale(local_panel, grid_wrap, number_of_lines, em_size, labelled_axes, y_labels, "left");
    }
    if(options.rightY !== false)
    {
      add_Y_scale(local_panel, grid_wrap, number_of_lines, em_size, labelled_axes, y_labels, "right");
    }

    that.render = function(render_data, cell_class) {
      data = render_data;
      local_panel.selectAll(".data_cell").remove();
      local_panel.selectAll(".annotation_cell").remove();

       var xTop = null,
           xAxisTop = null,
           xBottom = null,
           xAxisBottom = null;
      if(annotation_key === true)
      {
         add_annotation_key(local_panel, data, cell_class, width, margin, number_of_lines, em_size, bottom_x_pad, heatmapColour, colour_scale, user_colours);
      }

      var cells = local_panel.selectAll(".item")
          .data(data);
      ...
      cells.enter().append("g")
          ...
          .attr("transform", function(d, i) {
            ...
            x_offset = (i * em_size) - (row_idx * grid_wrap * em_size);
            y_offset = row_idx * em_size;
            return("translate("+x_offset+","+y_offset+")");
          });

          text_x_offset = em_size - (em_size/3);
          text_y_offset = (em_size / 2)-(em_size/100);

          place_cells(cells, em_size, cell_class, user_colours, heatmapColour, colour_scale);
    ...
    that.getSVG = function(){return svg_div_class;};
    that.getGridwrap = function(){return grid_wrap;};
    that.getData = function() {return data;};
    that.getDataLength = function() {return data_length;};
    that.getNumberOfLines = function() {return number_of_lines;};
    that.getEmSize = function() {return em_size;};
    that.getHeight = function() {return height;};
    that.getWidth = function() {return width;};
    that.getTopXPad = function() {return top_x_pad;};
    that.getBottomXPad = function() {return bottom_x_pad;};
    that.getLeftYPad = function() {return left_y_pad;};
    that.getRightYPad = function() {return right_y_pad;};
    that.getMargin = function(){return margin;};

    return that;
}

//convenience method for building panels of sequence annotation
...
module.exports = {
  ...
  bio_panel: bio_panel,
  process_sequence_string: process_sequence_string,
  add_annotation: add_annotation,
};

},{"./d3.v3.min.js":1,"msa-colorschemes":9}],3:[function(require,module,exports){
module.exports = {
  A: "#00a35c",
  R: "#00fc03",
  N: "#00eb14",
...
},{"./buried":3,"./cinema":4,"./clustal":5,"./clustal2":6,"./helix":7,"./hydrophobicity":8,"./lesk":10,"./mae":11,"./nucleotide":12,"./pid_colors.js":13,"./purine":14,"./schemeclass":15,"./strand":16,"./taylor":17,"./turn":18,"./zappo":19}],10:[function(require,module,exports){
module.exports = {
  A: " orange",
  B: " #fff",
  C: " green",
